...
Para reducir falsos positivos y pruebas inestables, cada perfil del simulador (éxito, error, límite) se ejecutará con \textbf{control temporal determinista}. En Jest se emplearán \texttt{fake timers} (modo moderno) y avance explícito del reloj virtual (\texttt{advanceTimersByTime}/\texttt{runAllTimers}); en Jasmine se usará el reloj simulado equivalente (\texttt{jasmine.clock()}) cuando corresponda. Además, las respuestas asíncronas del simulador se encapsularán en una cola controlada de microtareas/macrotareas para evitar desfases del event loop entre corridas. El criterio de aceptación para estabilidad será repetir lotes de pruebas en más de una corrida consecutiva sin variación de estado (sin fallas intermitentes atribuibles al orden temporal).
...
Si durante la ejecución aparece un plugin Capacitor no mapeado explícitamente en el núcleo (\texttt{geolocation}, \texttt{camera}, \texttt{preferences}), el sistema aplicará un \textbf{mecanismo de caída segura}: generación dinámica de un mock por defecto mediante \texttt{Proxy} de JavaScript, con respuestas asíncronas genéricas y trazadas en log. Este fallback evita que la suite se detenga por razones técnicas ajenas al alcance y permite etiquetar esos casos como ``pendientes de implementación específica'' en lugar de romper la validación principal.

\subsubsection{Justificación operativa de MCP frente a automatización clásica}
\label{ssec:justificacion_mcp_operativa}
El uso de MCP no se limita a ejecutar comandos que también podría correr un script. La diferencia propuesta es \textbf{orquestación semántica}: un agente puede leer el estado del reporte de cobertura (líneas/ramas faltantes, suites fallidas, tipo de error) y seleccionar de forma guiada el siguiente perfil de simulación a probar (éxito, error, límites) antes de la siguiente corrida. En ese contexto, MCP aporta un protocolo de contexto entre herramientas y agente, mientras que Bash/CI aporta solo secuencias fijas. La tesis evaluará esta capa como apoyo adaptativo, no como sustituto de la instrumentación base.
